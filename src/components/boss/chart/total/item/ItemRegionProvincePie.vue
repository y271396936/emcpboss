  ...
  <div class="ms-list">
    <el-row class="operation">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>省份占比图</el-breadcrumb-item>
      </el-breadcrumb>
    </el-row>
...
<script>
  ...
  import { PROVINCE_SEARCH_CONDITION } from '@/models/report/item-region-consumption'
  ...
  export default {
    ...
    created () {
      this.search.list = PROVINCE_SEARCH_CONDITION
    ...
    methods: {
      requestList () {
        ...
        Api.getProvincePie({search: search}, response => {
          let status = response.status || 0
          let data = response.data || {}
          if (status === 200 || status === 204) {
            this.chartData = data.map.data
            this.title = response.data.map.title
          } else {
