  ...
  <div class="ms-list">
    <el-row class="operation">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>项目地域分析(历史数据)</el-breadcrumb-item>
      </el-breadcrumb>
    </el-row>
    ...
    <el-table
      ...
      :data="recombineList"
      :fit="true">
      <el-table-column prop="day" label="日期" min-width="70" fixed="left"></el-table-column>
...
<script>
  ...
  import Api from '@/api/backup/backup-item-region-consumption'
    ...
    created () {
      ...
      this.search.list = [
        ...
        },
        {
          ...
          config: {
            list: [
              {value: 1, label: '按 项目-省-市 合并'},
              {value: 2, label: '按 项目-账户-省-市 合并'},
              {value: 3, label: '按 项目-省 合并'}
            ]
          }
        },
        {
    ...
    computed: {
      recombineList () {
        ...
        this.table.list.forEach(item => {
          ...
          if (timeShare.province === undefined || timeShare.province === '') {
            timeShare.province = '_'
          }
          if (timeShare.city === undefined || timeShare.city === '') {
            timeShare.city = '_'
          }
          ...
          if (this.search.condition.dataGroup === 1) {
            timeShare.industry = '_'
            timeShare.channelAccountName = '_'
          }
          ...
          if (item.hasOwnProperty('clickRatio')) {
            timeShare.clickRatio = parseFloat(item.clickRatio).toFixed(2) + '%'
          } else {
            timeShare.clickRatio = '_'
          }
          if (item.hasOwnProperty('clickPrice')) {
            timeShare.clickPrice = parseFloat(item.clickPrice).toFixed(2)
          } else {
            timeShare.clickPrice = '_'
          }
          if (item.hasOwnProperty('translationCost')) {
            timeShare.translationCost = parseFloat(item.translationCost).toFixed(2)
          } else {
            timeShare.translationCost = '_'
          }
          list.push(timeShare)
        })
        return list
