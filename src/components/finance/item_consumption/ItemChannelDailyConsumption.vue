  ...
  <div class="ms-list">
    <el-row class="operation">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>项目渠道消费日报</el-breadcrumb-item>
    ...
    <el-table
      ...
      :fit="true">
      <el-table-column prop="title" label="项目" min-width="120" fixed/>
      <el-table-column prop="platformTitle" label="平台" min-width="120" fixed/>
      <el-table-column prop="typeId" label="渠道" min-width="120">
...
<script>
  ...
  import Api from '@/api/report/item-channel-daily'
  import ResponseParse from '@/utils/response-parse'
  import { PLATFORM_LIST } from '@/utils/constants'
  import { SEARCH_CONDITION } from '@/models/report/item-channel-daily'
    ...
    computed: {
      recombineList () {
        ...
        for (let key in this.table.list) {
          let arrayData = {}
          arrayData['title'] = key.split('-')[0]
          arrayData['platformTitle'] = platforms[key.split('-')[1]]
          arrayData['typeId'] = key.split('-')[2]
          let total = 0
          for (let n = 1; n <= 31 + 1; n++) {
            let a = (this.search.condition.year * 10000 + this.search.condition.month * 100 + n).toString()
            if (this.table.list[key][a] !== undefined) {
              ...
              if (searchValue === '总消费点数') {
                ...
                arrayData[n] = this.table.list[key][a]['profit']
