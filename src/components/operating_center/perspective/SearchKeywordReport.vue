  ...
  <div class="ms-list">
    <el-row class="operation">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>搜索词数据列表</el-breadcrumb-item>
      </el-breadcrumb>
    </el-row>
    ...
    <el-table
      ...
      <el-table-column prop="itemTitle" label="项目名称" min-width="100" fixed>
        <template slot-scope="scope">
          <a href="javascript:;" @click="searchCriteria(scope.row.itemTitle, index=7)">{{scope.row.itemTitle}}</a>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="channelTitle" label="渠道" min-width="80"></el-table-column>
      ...
      <el-table-column prop="unitTitle" label="推广单元" min-width="105"></el-table-column>
      ...
      <el-table-column prop="searchKeyword" label="搜索词" min-width="100">
        <template slot-scope="scope">
          <a href="javascript:;" @click="searchCriteria(scope.row.searchKeyword, index=5)">{{scope.row.searchKeyword}}</a>
        </template>
      </el-table-column>
      <el-table-column prop="unitStatus" label="当前单元的添加状态" min-width="60">
        <template slot-scope="scope">
          <span v-if="scope.row.unitStatus === 1">已添加</span>
          <span v-else-if="scope.row.unitStatus === 0">未添加</span>
          <span v-else>未知</span>
        </template>
      </el-table-column>
...
<script>
  ...
  import Api from '@/api/report/search-keyword-report'
  import LogApi from '@/api/report/perspective-report-import-log'
  import ResponseParse from '@/utils/response-parse'
  import { SEARCH_CONDITION } from '@/models/report/search-keyword-report'
...
//            this.tbHeight = ''
            this.table.list = []
            this.$message.error(response.data.error || '搜索词数据获取失败')
          }
