<template>
  <div class="ms-list">
    ...
    <el-table
      ...
      <el-table-column prop="unitTitle" label="推广单元" min-width="130"></el-table-column>
      <el-table-column prop="keyword" label="关键词" min-width="130">
        <template slot-scope="scope">
          <a href="javascript:;" @click="searchCriteria(scope.row.keyword, index=6)">{{scope.row.keyword}}</a>
        </template>
      </el-table-column>
      <el-table-column prop="keywordTag" label="关键词代码" min-width="120"></el-table-column>
      <el-table-column prop="keywordCategoryTitle" label="品类" min-width="80">
        <template slot-scope="scope">
          <a href="javascript:;"
             @click="searchCriteria(scope.row.keywordCategoryTitle, index=7)">{{scope.row.keywordCategoryTitle}}</a>
        </template>
      </el-table-column>
      <el-table-column prop="keywordNatureTitle" label="词性" min-width="80">
        <template slot-scope="scope">
          <a href="javascript:;"
             @click="searchCriteria(scope.row.keywordNatureTitle, index=8)">{{scope.row.keywordNatureTitle}}</a>
        </template>
      </el-table-column>
      <el-table-column prop="keywordMode" label="匹配模式" min-width="70"></el-table-column>
      <el-table-column prop="keywordPrice" label="出价" min-width="60" align="right" header-align="center">
        <template slot-scope="scope">
          {{scope.row.keywordPrice.toFixed(2)}}
        </template>
      </el-table-column>
      <el-table-column prop="keywordPcScore" label="计算机质量度" min-width="60" align="right"
                       header-align="center"></el-table-column>
      <el-table-column prop="keywordMobileScore" label="移动质量度" min-width="60" align="right"
                       header-align="center"></el-table-column>
      ...
      <el-table-column prop="clickRatio" label="点击率" min-width="80" align="right" header-align="center">
        <template slot-scope="scope">
          <span v-if="search.condition.dataGroup !== 2">{{scope.row.clickRatio.toFixed(2)}}%</span>
          <span
            v-else-if="scope.row.impression !== 0">{{(scope.row.click / scope.row.impression * 100).toFixed(2)}}%</span>
          <span v-else="">∞</span>
        </template>
      </el-table-column>
      <el-table-column prop="clickPrice" label="平均点击价格" min-width="80" align="right" header-align="center">
        <template slot-scope="scope">
          <span v-if="search.condition.dataGroup !== 2">{{scope.row.clickPrice.toFixed(2)}}</span>
          <span v-else-if="scope.row.click !== 0">{{(scope.row.costCoin / scope.row.click).toFixed(2)}}</span>
          <span v-else="">∞</span>
        </template>
      </el-table-column>
...
<script>
  ...
  import Api from '@/api/report/keyword-report'
  import LogApi from '@/api/report/perspective-report-import-log'
  import ResponseParse from '@/utils/response-parse'
  import { SEARCH_CONDITION } from '@/models/report/keyword-report'
...
//      },
      requestList () {
        ...
        this.search.condition.startTime = this.search.condition.date
        this.search.condition.endTime = this.search.condition.date
...
//            this.tbHeight = ''
            this.table.list = []
            this.$message.error(response.data.error || '关键词汇总数据获取失败')
          }
