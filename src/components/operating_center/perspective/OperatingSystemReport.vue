  ...
  <div class="ms-list">
    <el-row class="operation">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>操作系统数据列表</el-breadcrumb-item>
      </el-breadcrumb>
    </el-row>
    ...
    <el-table
      ...
      <el-table-column prop="platformId" label="平台" min-width="80">
        <template slot-scope="scope">
          <ms-platform :platform-id="scope.row.platformId"></ms-platform>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="planTitle" label="推广计划" min-width="220"></el-table-column>
      <el-table-column prop="phonePlatform" label="操作系统" min-width="120">
        <template slot-scope="scope">
          <span v-if="scope.row.phonePlatform === 1">Android</span>
          <span v-else-if="scope.row.phonePlatform === 2">IOS</span>
          <span v-else-if="scope.row.phonePlatform === 3">Windows Phone</span>
          <span v-else-if="scope.row.phonePlatform === 4">计算机</span>
          <span v-else-if="scope.row.phonePlatform === 5">Web</span>
...
<script>
  ...
  import Api from '@/api/report/operating-system-report'
  import LogApi from '@/api/report/perspective-report-import-log'
  import ResponseParse from '@/utils/response-parse'
  import { SEARCH_CONDITION } from '@/models/report/operating-system-report'
...
//            this.tbHeight = ''
            this.table.list = []
            this.$message.error(response.data.error || '地域数据获取失败')
          }
