  ...
  <div class="ms-list">
    <el-row class="operation">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>兴趣分类数据列表</el-breadcrumb-item>
      </el-breadcrumb>
    </el-row>
    ...
    <el-table
      ...
      <el-table-column prop="interestType" label="兴趣分类" min-width="120">
        <template slot-scope="scope">
          <span v-if="scope.row.interestType.indexOf('$') === -1">{{ scope.row.interestType }}</span>
          <span v-else>
            一级兴趣: {{ scope.row.firstType }}
            二级兴趣: {{ scope.row.secondType }}
          </span>
        </template>
      </el-table-column>
...
<script>
  ...
  import Api from '@/api/report/interest-type-report'
  import LogApi from '@/api/report/perspective-report-import-log'
  import ResponseParse from '@/utils/response-parse'
  import { SEARCH_CONDITION } from '@/models/report/interest-type-report'
      ...
      requestList () {
        ...
        Api.list({search: search}, response => {
          ...
          if (status === 200 || status === 204) {
            body.forEach(item => {
              if (item.interestType.indexOf('$') !== -1) {
                item.firstType = item.interestType.split('$')[0]
                item.secondType = item.interestType.split('$')[1]
              }
            })
