<template>
  <div class="ms-list">
    <el-row class="operation">
      ...
      <el-button-group>
        <el-button type="primary" size="small" @click="addRow()">发布</el-button>
      </el-button-group>
    </el-row>
    ...
    <ms-slide
      ...
      <el-form class="form" label-width="120px" size="small" :model="row.form" ref="formModifyRow">
        ...
        <el-form-item label="类别" prop="marketingAssortentId">
          <template slot-scope="scope">
            <el-select  v-model="row.form.marketingAssortentId" placeholder="请选择">
              <el-option
        ...
        <el-form-item>
          <el-button type="primary" size="small" style="" @click="showData">
            有效时间
          </el-button>
        </el-form-item>
        <el-form-item label="活动时间" v-if="showTime">
          <div class="block">
            <el-date-picker
    ...
    <div v-show="showList">
      ...
      <el-table
        ...
        <el-table-column prop="name" label="类别" min-width="80">
        </el-table-column>
        ...
        <el-table-column prop="updator" label="修改人" min-width="80">
        </el-table-column>
        <el-table-column prop="updatorTime" label="修改时间" min-width="140"></el-table-column>
    ...
    <div class="detail" v-show="!showList">
      <div class="back" @click="backDetail()">
        <icon name="angle-left" scale="2" class="icon-bg"></icon>
      </div>
      <!--<div class="detail-scroll" ref="detail" :style="detailStyle">-->
...
<script>
  ...
  import Api from '@/api/platform/activity-released'
  ...
  import { SEARCH_CONDITION, FormArgument, FileFormArgument } from '@/models/platform/activity-released'
        ...
        this.$refs['formModifyRow'].validate((valid) => {
          if (valid) {
            if (this.row.edit.enabled) {
              Api.modify({id: this.table.list[this.row.edit.index].id}, {form: this.row.form}, response => {
                let status = response.status || 0
                let body = response.data || []
                if (status >= 200 && status <= 204) {
              ...
              this.row.form.startActivityTime = this.activityDate[0]
              this.row.form.endActivityTime = this.activityDate[1]
              Api.insert({form: this.row.form}, response => {
                let status = response.status || 0
                let body = response.data || []
                if (status >= 200 && status <= 204) {
                  this.table.list.forEach(item => {
                    if (item.marketingAssortentId === body.marketingAssortentId) {
                      body.name = item.name
                    }
                  })
                  body.display = 0
                  body.defaultDisplay = 0
