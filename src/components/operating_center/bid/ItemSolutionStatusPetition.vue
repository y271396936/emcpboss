  ...
  <div class="ms-list">
    <el-row class="operation">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>项目方案上下稿申请</el-breadcrumb-item>
        <el-breadcrumb-item>列表</el-breadcrumb-item>
      </el-breadcrumb>
    </el-row>

    <ms-slide title="拒绝上下稿申请" :width="500" @close="closeSlide()" ref="slideRepulse">
      <el-form class="form" label-width="120px" size="small" :model="row.form" ref="formRepulse">
        ...
        <el-form-item label="操作类型" prop="genre">
          <el-select v-model="row.form.genre" :disabled="true">
            <el-option label="下稿" :value="1"></el-option>
            <el-option label="上稿" :value="2"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="申请理由" prop="reason">
          <el-input v-model="row.form.reason" type="textarea" :disabled="true"></el-input>
        </el-form-item>
    ...
    <el-table v-loading="table.loading" size="small" stripe border :data="table.list" :fit="true">
      ...
      <el-table-column prop="itemTitle" label="项目名称" min-width="130" fixed></el-table-column>
      ...
      <el-table-column prop="genre" label="申请类型" min-width="80">
        <template slot-scope="scope">
          <span v-if="scope.row.genre === 1" class="ms-danger">下稿</span>
          <span v-else-if="scope.row.genre === 2" class="ms-success">上稿</span>
          <span v-else>未知</span>
        </template>
      </el-table-column>
      <el-table-column prop="regulator" label="运营专员" min-width="80"></el-table-column>
      <el-table-column prop="operator" label="申请人" min-width="80">
        <template slot-scope="scope">
          <a href="javascript:;" @click="searchCriteria(scope.row.operator, index=2)">{{scope.row.operator}}</a>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="approver" label="审批人" min-width="80"></el-table-column>
...
<script>
  ...
  import Api from '@/api/platform/item-solution-status-petition'
  import ResponseParse from '@/utils/response-parse'
  import { FormArgument, SEARCH_CONDITION } from '@/models/platform/item-solution-status-petition'
      ...
      approve () {
        Api.approve({
          ...
          if (status >= 200 && status <= 204) {
            ...
            this.$message.success('已同意该项目方案的下稿申请')
          } else {
